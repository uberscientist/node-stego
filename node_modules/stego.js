...
function bmp_info(data){
  ...
  info.row_size = ((info.bit_depth*info.img_w)/32)*4;

  //pad row_size if needed, must be a multiple of a 32-bit DWORD
  if(info.row_size % 4 != 0){
     info.row_size += 2
     info.row_padding = true;
  } else {
    info.row_padding = false;
  }
  return info;
}
...
function encode(secret, image_file, callback){
  ...
  child = exec('convert '+image_file+' '+image_file+'.bmp', function(err, stdout, stderr){
    if(err) throw err;
    ...
    fs.readFile(secret, function(err, secret_data){
      if(err) throw err;
        ...
        fs.readFile(image_file, function(err, data){
          ...
          if(secret_data.length >= bmp.pixelArray_length/3){
            callback('Secret file is <strong>'+ Math.floor(secret_data.length/1000)+
            'kB</strong>, it needs to be <strong>&lt;'+
            Math.floor(bmp.pixelArray_length/3)/1000+'kB</strong> to fit in your image', 'none');
            return;
          }
          ...
          for(var i=0; i< nib_secret.length; i++){
            if(i != 0 && bmp.row_padding == true && i*3 % (bmp.row_size-2) == 0){
              //skip padding to keep alignment
              skip=2;
            }

            var byte_pos = bmp.pixel_start+(i*3)+skip+off;
            stego_val = (data[byte_pos] << 4) | nib_secret[i];
